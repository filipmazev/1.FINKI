...
public:
	...
	friend std::istream& operator >> (std::istream& input, ComplexNumber& c) noexcept { input >> c.real >> c.imaginary;	return input; }
...
public:
	Equation() { this->arr = nullptr; this->operation = nullptr; this->num=0; }

	Equation(const Equation& e):Equation() {
		if (this != &e) {
			delete[] this->arr; this->arr = new ComplexNumber[e.num];
			delete[] this->operation; this->operation = new char[strlen(e.operation) + 1];
			strcpy(this->operation, e.operation);
	...
	friend std::istream& operator >> (std::istream& input, Equation& e) noexcept {
		...
		char op='/'; while (op != '='){
			...
			char* c_tmp = new char[(op!='='?e.num+1:e.num)]; strncpy(c_tmp, e.operation, e.num);
