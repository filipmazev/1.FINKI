...
public:
    ...
    Automobile(char* brand, int* registry, int max_speed) {
        ...
        strcpy(this->brand, brand); for (int i = 0; i < REG_LENGHT; i++) { this->registry[i] = registry[i]; }
    }
    ...
    Automobile(const Automobile& a) {
        if (this != &a) {
            ...
            strcpy(this->brand, a.brand); for (int i = 0; i < REG_LENGHT; i++) { this->registry[i] = a.registry[i]; } this->max_speed = a.max_speed;
        }
    }
...
public:
    ...
    RentACar(char* name) { this->name = new char[strlen(name) + 1]; strcpy(this->name, name); }
